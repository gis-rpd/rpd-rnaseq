...
* [TrimGalore](#trimgalore) - adapter trimming (optional)
* [STAR](#star) - alignment
* [RSEM](#rsem) - Estimate gene and isoform expression from RNA-Seq data
...
All results are written to the so called publish directory, which is `results` by default (and used
as example below).

Many results (FastQC, STAR alignment stats etc.) are summarised in MultiQC plots, which
can be found in `results/MultiQC/multiqc_report.html`. An [example MultiQC report can be download
...
See [MultiQC homepage](http://multiqc.info)
...
[TrimGalore](http://www.bioinformatics.babraham.ac.uk/projects/trim_galore/) is used for (optional) removal of adapter contamination and trimming of low quality regions. TrimGalore uses [Cutadapt](https://github.com/marcelm/cutadapt) for adapter trimming and runs FastQC after it finishes.
...
See [TrimGalore homepage](http://www.bioinformatics.babraham.ac.uk/projects/trim_galore/)
...
The STAR section of the MultiQC report shows a bar plot with alignmen:wt rates: good samples should have most reads as _Uniquely mapped_ and few _Unmapped_ reads.
...
![RSEM mapped reads](images/rsem_assignment_plot.png)


![RSEM multimapping rates](images/rsem_multimapping_rates.png)
...
### RSEM documentation:
- [rsem-calculate-expression](https://deweylab.github.io/RSEM/rsem-calculate-expression.html)
- [rsem-plot-model](https://github.com/deweylab/RSEM)
...
![Picard RnaSeqMetrics read assignments](images/picard_rnaseqmetrics_assignment_plot.png)


![Picard RnaSeqMetrics normalized gene coverage](images/picard_rna_coverage.png)
